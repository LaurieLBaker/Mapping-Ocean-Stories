...
## Extracting keyword + words of the same sentence before and after
```{r extract_keyword}
# Choose a  keyword
...
extract_keyword_and_context <- function(text, keyword) {
  keyword_index <- regexpr(paste0("(?i)\\b", keyword, "\\b"), text)
  
  if (keyword_index == -1) {
    return(list(before = NULL, after = NULL))
  }
  
 before <- sub(".*\\. ", "", substr(text, 1, keyword_index + attr(keyword_index, "match.length") - 1))
  after <- sub(paste0(".*(?i)\\b", keyword, "\\b(.*?)(\\.|$)"), "\\1", text)
  
  return(list(keyword = keyword, before = before, after = after))
}
...
# Process data with the new keyword so that we can change it
...
word_document_freq <- lawson1_word %>%
  group_by(document) %>%
  mutate(tf = n / sum(n))

total_documents <- n_distinct(lawson1_word$document)
lawson1_word_idf <- lawson1_word %>%
  group_by(word) %>%
  summarise(idf = log(total_documents / sum(document %in% unique(document)))) 

lawson1_word_tf_idf <- word_document_freq %>%
  left_join(lawson1_word_idf, by = "word") %>%
  mutate(tf_idf = tf * idf) 

print(lawson1_word_tf_idf)
...
```{r word_cloud, fig.height=5, fig.width=5}
wordcloud(
