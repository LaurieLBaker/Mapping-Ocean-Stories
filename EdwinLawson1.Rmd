...
## SpacyR location extraction EL1 (I believe we need to also have Python installed on computer with the SpaCy for spacyr to work. I don't have it, anyone does?)
...
word_freq_1 <- table(lawson1_word$word)
target_freq_1 <- word_freq_1[names(word_freq_1) %in% word_list]
target_freq_1 <- sort(target_freq_1, decreasing = TRUE)
word_freq_1 <- as.data.frame(word_freq_1)
target_freq_1 <- as.data.frame(target_freq_1)

colnames(word_freq_1) <- c("word", "freq")
colnames(target_freq_1) <- c("word", "freq")

target_freq_1 <- target_freq_1[order(-target_freq_1$freq), ]
```

```{r word_cloud, fig.height=15, fig.width=15}
wordcloud(
  words = target_freq_1$word,
  freq = target_freq_1$freq,
  min.freq = 1,
